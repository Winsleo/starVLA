...
# Adapted for the upstream-rebase experiment (D-056): single-dataloader variant of
# train_vlajepa_video.py that reads `datasets.vla_data` (real LIBERO data via
# dataloader/jepa_lerobot_datasets.py) instead of `datasets.video_data` (an action-free video
# dataset not available on this site). See docs/plans/upstream-rebase-experiment.md.
#
# Everything else -- the accelerate/DeepSpeed setup, VLATrainer shape, and split_loss_terms-based
# _train_step that sums action_loss + wm_loss -- is unchanged from train_vlajepa_video.py.
# Deliberately NOT based on train_starvla.py: that entrypoint's _train_step reads only
# output_dict["action_loss"], which would silently drop VLA_JEPA's world-model loss.
...
def prepare_data(cfg, accelerator, output_dir) -> Tuple[DataLoader, DataLoader]:
    ...
    # VLA data loader (real LIBERO data, dataset_py=jepa_lerobot_datasets)
    logger.info(f"Creating VLA Dataset with Mixture `{cfg.datasets.vla_data.dataset_py}`")
